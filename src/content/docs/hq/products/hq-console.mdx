...
- **Agents** — per-agent settings, including an **Owners** panel (list, invite, and revoke agent owners) shown only to an agent's active owners. Agent ownership is what powers owner delegation when an owner messages the agent in [Slack](/hq/products/capabilities/agents-in-slack/).
- **Personal Integrations** — per-user account links, such as connecting a Slack workspace whose email differs from your HQ login so agents recognize you there (see [Agents in Slack](/hq/products/capabilities/agents-in-slack/)).
