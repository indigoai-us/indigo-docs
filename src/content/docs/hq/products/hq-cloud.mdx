...
**hq-cloud** is the sync engine of the HQ ecosystem, published to npm as **`@indigoai-us/hq-cloud`** (currently **6.x**). It does the actual work of keeping a local HQ instance synchronized with cloud storage.
...
- **Conflict resolution** via a local journal that tracks per-file sync state. When both sides changed, the higher frontmatter `version:` wins (else the newer modification time); the losing copy is parked under `.hq/conflict-backups/` rather than written beside the live file. See [Cloud Sync](/hq/guide/8-cloud-sync/#conflict-resolution).
