...
- A system-tray agent with per-workspace sync rows, real per-file progress, a Stop button, conflict resolution, new-file notifications, instant DM notifications, a notification history window, and Connect diagnostics.
...
Notifications in HQ Sync are normally transient — a DM banner or a new-file alert appears and then it's gone. As of **0.6.0**, a bell button in the popover header opens a **Notifications** window: a single, reverse-chronological timeline (grouped by day) of everything HQ Sync has surfaced for you, so nothing you dismissed is lost.

It brings three notification types into one view:
