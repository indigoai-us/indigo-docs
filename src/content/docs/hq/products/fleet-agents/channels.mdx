...
2. Under **Channels**, expand **Slack** and choose **Attach Slack**.
...
Under **Channels**, expand **Slack** to see whether inbound events are healthy, outbound-only, waiting for an event, or missing configuration. The manager also shows the most recent Slack event seen by HQ.
...
In Agent Settings, expand **Channels → Telegram**, then:
