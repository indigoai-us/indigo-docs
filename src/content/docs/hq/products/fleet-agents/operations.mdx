...
In Agent Settings, expand **Channels → Slack**:
...
In Agent Settings, expand **Channels → Telegram** and check the caller group. For Slack or email, ask an operator to inspect any API-managed caller allowlist. A configured restriction rejects callers that HQ cannot map to an allowed active company member.
