...
hq agents config <agent-uid> --model gpt-5.6-terra --effort high --tier default
...
In Agent Settings, expand **Live systems → Connections → Slack**:
...
In Agent Settings, expand **Live systems → Connections → Telegram** and check the caller group. For Slack or email, ask an operator to inspect any API-managed caller allowlist. A configured restriction rejects callers that HQ cannot map to an allowed active company member.
