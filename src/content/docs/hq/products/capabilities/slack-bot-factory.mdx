...
> **The Slack Bot Factory is a managed surface of [hq-pro](/hq/products/hq-pro/about/): a one-time workspace connect, the in-Slack `/hq-new-bot` command, and the [hq-packages](/hq/products/hq-packages/) `hq-pack-slack-bot` runtime.** It mints real, per-bot Slack apps on demand and lands their tokens in your company vault.

:::caution[Workspace connect is currently white-glove]
The one-time workspace connect is **not yet self-serve** — there is no `/hq-slack-connect` command in the current HQ release. To connect a Slack workspace, **reach out to the Indigo team** ([hello@getindigo.ai](mailto:hello@getindigo.ai) or your shared Slack channel) and we'll run the connect with you. Once your workspace is connected, minting bots with `/hq-new-bot` and running them with `hq-pack-slack-bot` works exactly as described below. A self-serve connect command is on the roadmap; this page documents the flow it will follow.
:::
...
Connecting a workspace is an **admin/owner-only** setup that happens once per Slack workspace. It is currently **run with the Indigo team** (see the note above) and follows the four steps below. The connect resolves the company you're connecting the workspace to, confirms you hold an `admin` or `owner` role in it (the gate runs before any vault write), and then walks through four steps.
...
- the `/hq-new-bot` slash command → `https://hqapi.hq.computer/slack-apps/create`
- the OAuth redirect → `https://hqapi.hq.computer/slack-apps/install-callback`
...
Slack app config tokens expire, and signing secrets are sometimes rotated. To refresh a connected workspace, **re-run the connect with the Indigo team for the same workspace.** It is idempotent on the `team_id`:
