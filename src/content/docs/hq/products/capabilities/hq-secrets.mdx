...
> **hq-secrets is a capability of the [`hq` CLI](/hq/products/hq-cli/), not a standalone repo.** It operates against the encrypted vault provided by [hq-pro](/hq/products/hq-pro/about/).
...
Each company secret has an ACL with `read` / `write` / `admin` permissions. The creator is implicitly admin (non-revocable). New secrets are **restricted** by default (creator-only); an **open** flag grants company-wide `read`. Grants can target a human, an active HQ agent, or a **group** (group membership changes adjust access without touching the ACL). Agent email addresses are resolved to their stable `agt_*` identity, so renaming an agent does not orphan its grant.

```bash
hq secrets acl PROD/DB_PASSWORD                                   # who has access
hq secrets share PROD/DB_PASSWORD --with alice@example.com --permission read
hq secrets share PROD/DB_PASSWORD --with agent@company.hq-agents.com --permission read
...
- [hq-pro](/hq/products/hq-pro/about/) — the vault and identity it operates against
