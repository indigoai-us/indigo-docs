...
2. **Allowed prefixes** — an allowlist of patterns the connection can read. Type a pattern, press **Enter** (or click **Add**), and it becomes a removable chip. Patterns from your existing company vault appear as one-click **Suggestions from your vault**. At least one pattern is required.
   - Wildcards: trailing `*` matches anything under the prefix. A bare `knowledge/` is auto-normalized to `knowledge/*`.
