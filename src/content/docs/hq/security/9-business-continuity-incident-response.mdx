...
description: How HQ stays available, recovers from failures, and responds to security incidents — stated with both what is in place today and what is being formalized.
---
...
- HQ operates in a single region (`us-east-1`); see the honest note below regarding multi-region.
...
:::note[Honest note]
PITR/KMS-CMK coverage is being standardized across all secondary metadata/audit tables; a documented, regularly-tested restore runbook is being formalized as part of the SOC 2 program.
:::
...
:::caution[Single-region design]
HQ currently runs in a single AWS region with no cross-region replication or automated regional failover, and Indigo has not yet published formal RPO/RTO targets or completed a documented DR test. Establishing measured RPO/RTO targets, a tested DR runbook, and evaluating multi-region/cross-region backup are roadmap commitments (see [Compliance Roadmap](/hq/security/10-compliance-roadmap/)). The single-region design means a prolonged region-wide AWS outage would affect availability until recovery.
:::
...
2. Triage and classification — assess scope, severity, and whether customer data is affected.
...
4. Eradication and recovery — remove the root cause and restore normal operation from backups/versioned data where needed.
5. Notification — see below.
...
:::note[Honest note]
The incident-response plan is being formalized into a written, role-assigned, periodically-tested runbook as part of the SOC 2 program.
:::
