---
title: Testing
description: Testing strategy and QA processes across the HQ ecosystem repos.
---
...
### Tier 1: Unit Tests
Per-repo tests for individual functions and modules. Each repo runs its own Vitest suite in its own CI.
...
```bash
# inside a given repo (e.g. hq-cloud)
cd hq-cloud
npm test
```
...
Each repo has its own `ci.yml` workflow that runs on every PR and push to main:
1. `npm ci` — install deps
2. `npm run build` — build that repo
3. `npm run typecheck` — TypeScript checking
4. `npm test` — run that repo's tests

Tests must pass before merging to that repo's main.
