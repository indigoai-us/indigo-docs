...
Each pack carries its own `version` in `package.yaml` and publishes as `@indigoai-us/hq-pack-<name>`. Users install with `hq install @indigoai-us/hq-pack-<name>` — see [hq-packages](/hq/products/hq-packages/overview/).
