---
title: hq-cloud
description: Deep dive into @indigoai-us/hq-cloud — the S3 sync engine that powers bidirectional file synchronization between a local HQ instance and IndigoAI cloud storage.
---

`@indigoai-us/hq-cloud` is the most involved package in the HQ ecosystem. It handles authentication with IndigoAI, bidirectional file synchronization to S3, background daemon management, file change detection, and conflict resolution via a local journal. The CLI delegates all sync operations to this package.

> One repo in the split [HQ ecosystem](/hq/architecture/2-ecosystem/) — published as `@indigoai-us/hq-cloud`; also driven by the [hq-sync](/hq/products/hq-sync/) menubar app.
...
When a file changed on both sides since the last sync, the engine decides a **version-aware winner** (`pickWinner` in `src/cli/conflict.ts`) on both pull and push:

1. Both bodies carry a frontmatter `version:` integer → the higher version wins.
2. Otherwise the newer modification time wins (gaps under 2 s count as a tie, since S3 `LastModified` is whole seconds).
3. Identical bodies keep local; any remaining tie keeps the cloud copy.

If local wins, the local body stays live and the next push publishes it under an etag fence; if cloud wins, the cloud body is adopted. Either way the **losing body is parked out of tree** under `.hq/conflict-backups/<path>.<timestamp>-<machine><ext>` — `.hq/` is sync-ignored — so nothing is destroyed and no sibling `<file>.conflict-*` twin is written beside the live file. The decision (`winner`, `reason`, `localVersion`, `remoteVersion`) is recorded on the file's row in `.hq-conflicts/index.json` and on the `conflict` progress event.

There is no three-way merge or diff. The journal tracks which direction the last sync went (`"up"` or `"down"`) as an audit trail.

### Reconciling legacy twins

Earlier engine versions wrote the loser as a sibling twin. `reconcileLegacyConflictTwins()` (`src/lib/conflict-reconcile.ts`, surfaced as `hq sync doctor --reconcile-conflicts`) walks the HQ root — skipping `.git`, `node_modules`, `.hq`, `.hq-conflicts`, and `repos` — groups twins per live path, and decides once per live file: a twin is promoted only when its frontmatter `version:` is strictly higher than the live file's (a twin's mtime is its detection time, never promotion evidence). Every other twin is parked under `.hq/conflict-backups/` with the live file untouched; orphan twins with no live sibling are parked and reported for manual review rather than restored; symlinked or unreadable twins are refused for manual review. Dry-run by default, `--yes` applies, and it never touches the vault.
