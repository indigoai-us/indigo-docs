...
> The module system is how the split repos of the [HQ ecosystem](/hq/architecture/2-ecosystem/) and external knowledge plug into a single instance.
...
```mermaid
graph TD
    source["Source repo"] --> copy["Copy files"] --> target["Target path"]
    copy --> state["Sync state<br/>(per-file SHA256 hashes)"]
```
