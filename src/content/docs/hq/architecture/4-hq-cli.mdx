...
The `@indigoai-us/hq-cli` package is the ongoing management CLI for HQ. It handles two concerns: **module management** (adding, syncing, listing, and updating external module repos) and **cloud sync** (bridging to `@indigoai-us/hq-cloud` for S3-backed mobile access).
...
Built with [Commander.js](https://github.com/tj/commander.js), the CLI registers these top-level command groups:

```
hq modules <add|sync|list|update>   # Module management
hq sync <init|start|stop|status|push|pull|doctor>  # Cloud sync
hq mesh <check|start|progress|blocked|done|note|story|doctor>  # Work mesh
```
...
```mermaid
graph TD
    sync["hq modules sync"] --> each{"For each module<br/>in manifest"}
    each --> clone["Clone or fetch<br/>module repo"]
    clone --> strategy{"Apply strategy"}
    strategy -->|link| symlink["Create<br/>symlink"]
    strategy -->|merge| merge["Copy with<br/>conflict detection"]
    strategy -->|copy| copy["Direct<br/>copy"]
    symlink --> lock["Record commit SHA<br/>in modules.lock"]
    merge --> lock
    copy --> lock
```
...
The `hq sync` command group bridges to `@indigoai-us/hq-cloud` via dynamic imports. Each subcommand lazily loads the cloud package and delegates to its exported functions:
...
| `hq sync doctor --reconcile-conflicts [--yes] [--hq-root <path>]` | `syncDoctor({ reconcileConflicts: true })` | Reconcile legacy `.conflict-*` twin files against their live files (dry-run unless `--yes`); local-only, no vault access |

Dynamic imports (`await import("@indigoai-us/hq-cloud")`) mean the cloud package is only loaded when cloud commands are actually invoked. Module management commands work without `hq-cloud` installed.
