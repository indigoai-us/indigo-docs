...
HQ Cloud runs on AWS, defined entirely with [SST (Serverless Stack)](https://sst.dev). The infrastructure config provisions Cognito authentication, S3-backed storage, API Gateway routes, Lambda handlers, and the web delivery layer for the dashboard.
...
Authentication uses Amazon Cognito as the identity authority for the web dashboard and API tokens. The Console sign-in surface routes users through Cognito Hosted UI with configured external identity providers, including Google and Microsoft for workspace sign-in, while API routes continue to validate Cognito-issued JWTs.
...
- **External providers** — Google and Microsoft identities are federated into the same user pool, so provider choice does not change the downstream JWT issuer or API audience.

The primary Console `UserPoolClient` supports the web sign-in flow. Other product clients use the same Cognito authority only when they are explicitly configured for that surface.
