---
title: "Tenant Isolation & Multi-tenancy"
description: "How HQ ensures one organization can never access another's data — per-tenant bucket and key, scoped credentials, test-enforced."
---

**Last reviewed:** 2026-05-28  ·  **Document status:** Published

Tenant isolation is HQ's most important security property. A team AI operating system holds an organization's working context and brokers access to its data, so the guarantee customers most need is simple to state: **one organization can never read or write another organization's data.** This document explains how HQ enforces that guarantee.
...
| **Authorization** | Membership role + path-scoped grants, recomputed per request, never cached. |
| **Verification** | Blocking end-to-end tests in CI that assert cross-tenant access is denied. |

## 1. Dedicated storage per tenant
...
## 2. Dedicated encryption key per tenant
...
> **Trust model, stated honestly.** Per-tenant keys are **Indigo-managed** AWS KMS keys. Indigo can, by necessity, operate the service on customer data (for example, to sync and serve it). HQ does **not** currently offer customer-managed keys (BYOK) or end-to-end (client-side) encryption where Indigo cannot decrypt. BYOK and E2E options are noted on the roadmap in [Compliance Roadmap](/security/compliance-roadmap/). We would rather state this clearly than imply a zero-knowledge model we do not provide.

## 3. Access brokered by scoped, short-lived credentials
...
1. A request arrives bearing the caller's verified identity (a validated JWT; see [Identity & Access Management](/security/identity-access-management/)).
...
## 5. Capability-based external sharing

When a user shares data outside their organization, HQ issues an **encrypted, single-use, time-limited capability token** rather than broadening bucket access (see [Data Security & Encryption](/security/data-security-encryption/)). These tokens are pinned to specific paths and a maximum permission level (capped at `write`; `admin` is never grantable through a share), and are consumed atomically so they cannot be replayed.

## 6. Isolation at the local-agent layer
...
See [Shared Responsibility Model](/security/shared-responsibility-model/) for the division of duties on customer-controlled machines.

## 7. Verification: isolation is test-enforced
...
- Per-tenant keys are Indigo-managed (no BYOK / E2E today) — see §2.
...
*This document is a point-in-time description of HQ's tenant-isolation architecture as of the date above, provided for security evaluation.*
